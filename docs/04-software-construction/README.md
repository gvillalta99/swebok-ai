...
Construction para a era dos Large Language Models (LLMs). Enquanto o SWEBOK v4.0
focava em "codificação manual, testes unitários e debugging", a versão 5.0
reconhece que **a construção de software tornou-se primariamente um processo de
orquestração e curadoria de código gerado por sistemas autônomos**.
...
A nova construção de software abandona a premissa de que codificação manual é a
atividade central, assumindo que **geração algorítmica é infraestrutura, não
produto**. O foco desloca-se de "como escrever código eficiente" para "como
especificar, gerar, verificar e integrar código de forma confiável".
...
*SWEBOK-AI v5.0 — Capítulo 4: Orquestração e Curadoria de Código* *Reescrito em:
2025-01-31*
