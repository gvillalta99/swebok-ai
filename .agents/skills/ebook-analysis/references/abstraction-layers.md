...
This reference defines the five abstraction layers used to classify concepts
from most abstract (Layer 0) to most concrete (Layer 4). Use these definitions
...
**Definition:** Universal principles that apply across domains and cultures.
These are the bedrock truths about human nature, systems, or reality.
...
**Definition:** Domain-specific theory that explains how the foundational
principles manifest in a particular field. These are the "laws" of the domain.
...
**Definition:** Approaches, frameworks, and models that translate theory into
actionable direction. These provide structure for how to think about problems in
the domain.
...
**Definition:** Specific methods, techniques, and practices that implement
strategies. These are concrete enough to be directly actionable but not tied to
specific tools.
...
**Definition:** Concrete implementations tied to specific tools, platforms, time
periods, or contexts. These are the most actionable but also the most likely to
become outdated.
...
If extraction is heavily weighted to one layer, review for missed concepts at
other layers.
...
This is guidance, not rule. A Layer 0 mechanism exists ("Reciprocity operates
through obligation"), as does a Layer 4 pattern ("The onboarding flow: welcome
email → profile setup → first action").
